...
# IMOPRTANT: run for at least 2 iterations. The first iteration has odd behavior during scoreVectors
'''
...
class KMeans():
	...
	def scoreVectors(self, example, center, centerMag):
		...
		dtpdt = dot_product(example, center) #from utils.py (copied from 221 assinment folder)
		magA = self.magnitude(example)
	...
	def selectRandomCenters(self, examples):
	    centers = []
	...
	def findLoss(self, assignments, examples):
		loss = 0
		for i, example in enumerate(examples):
			center = self.centers[assignments[i]]
			centerMag = self.magnitude(center)
			loss += math.pow(self.scoreVectors(example[0], center, centerMag), 2)
		return loss

	#Finds the closest center to the given example
	def findGroup(self, example):
		minimum = -1
		index = 0
		for x in range(len(self.centers)):
			center = self.centers[x]
			centerMag = self.magnitude(center)
			score = self.scoreVectors(example, center, centerMag)
	...
	def learn(self, featureVectors):
		...
		self.centers = self.selectRandomCenters(featureVectors) #selects randomly from our feature vectors 
		for i in range(self.maxIters):
			...
			for j in range(0, len(featureVectors)):
				...
				groupIndex = self.findGroup(features) 
			...
			#update centers
			for x in range(self.k):
				self.centers[x] = self.findMiddle(z[x])
			cost = self.findLoss(assignments, featureVectors) 
		...
		return self.centers, assignments, cost, z
	...
	def test(self, example):
		...
		groupIndex = self.findGroup(example)
		return self.classifiedCenters[groupIndex]
...
def main():
	...
	centers, assignments, cost, z = km.learn(trainingExamples)

	print 'Finished Training'
	print 'Final Cost: ' + str(cost)
	print 'Begin classification... '

	km.classifyCenters(z)
	evaluate(pickledTestingFile, km.test)
